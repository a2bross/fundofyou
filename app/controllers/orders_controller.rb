class OrdersController < ApplicationController
  ...
  def create
    ...
    if @order.save
      @donation1 = Donation.new(project: @project1, user: @user, order: @order, amount: @order.amount.div(@project_nb), status: 1)
      @donation2 = Donation.new(project: @project2, user: @user, order: @order, amount: @order.amount.div(@project_nb), status: 1)
